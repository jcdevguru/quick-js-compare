...
describe('successful compare', () => {
  const testLabel = (testName: string, options?: Option) => {
    if (!options) {
  ...
  const testMatchingCompare = (testName: string, left: Value, right: Value, options?: Option) => {
  ...
  const testMismatchingCompare = (testName: string, left: Value, right: Value, options?: Option) => {
    test(testLabel(testName, options), () => {
      const c = new Compare(options);
      const r = c.compare(left, right).result;
      expect(r).toBeInstanceOf(Object);
